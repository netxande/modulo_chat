class ConsumersController < ApplicationController
  ...
  def token
    segredo = Digest::MD5.hexdigest("#{Faker::Name.name}#{Time.now}")
    consumer = Consumer.find(params[:format])
    consumer.update({:token => "#{segredo}"})
    #redirect_to action: 'index'
    redirect_to({ action: 'index' }, notice: "Token do #{consumer.description} gerado com sucesso!")
  end
  ...
  private
    ...
    def consumer_params
      params.require(:consumer).permit(:description, :author, :token)
    end
end
